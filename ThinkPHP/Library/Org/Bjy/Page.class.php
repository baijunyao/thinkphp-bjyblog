...
class Page {
    ...
    private function UrlParameters($url = array()) {
        ...
        if ($this->Static) {
        	...
        	if($this->searching){
        		$url=$this->leuu(MODULE_NAME  . "/" . CONTROLLER_NAME . "/" . ACTION_NAME)."?".http_build_query ($arg);
        	}else{
        		$url=$this->leuu(MODULE_NAME  . "/" . CONTROLLER_NAME . "/" . ACTION_NAME,$arg);
        	}
...
public function leuu($url='',$vars='',$suffix=true,$domain=false){
